...
double Floorplanner::computeCost() {
    ...
    return _alpha * _chipHeight * _chipWidth + (1 - _alpha) * _wireLength + 1000 * penalty;
}
...
void Floorplanner::simulatedAnnealing() {
    ...
    double ratio = 0.9;
    while (Temp > 1) {
        if (Temp > 1000)  // first stage: high temperature random search
            ratio = 0.99;
        else if (Temp > 100) // pseudo-greedy local search
            ratio = 0.999;
        else {
            if (!faFlag) {
                Temp = 10000;
        ...
        switch (op) {
            ...
            case 1: {  // delete and insert a macro in anyplace
                // break;
                ...
                if (targetNode->getLChild() == NULL && targetNode->getRChild() == NULL) {
                    if (leftOrRight == 0) {
...
void Floorplanner::writeResult(fstream& outFile) {
    outFile << _alpha * _chipHeight * _chipWidth + (1 - _alpha) * _wireLength << '\n';
    outFile << _wireLength << '\n';
